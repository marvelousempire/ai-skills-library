...
- **85** total `SKILL.md` files under [`skills/`](skills/).
...
| **post-ship-elevation-pass** | Claude Code + Cursor | [`skills/engineering/post-ship-elevation-pass/`](skills/engineering/post-ship-elevation-pass/) | After every substantive ship: gap audit (every incomplete/deferred/fragile item) + elevation pass (most ambitious version). Produces labeled lists. Waits for sign-off before implementing elevations. | "Use **post-ship-elevation-pass**." or the skill fires after every ship automatically | — |
| **plan-first-substantive-changes** | Claude Code + Cursor | [`skills/engineering/plan-first-substantive-changes/`](skills/engineering/plan-first-substantive-changes/) | Every change touching >2 files gets a plan document at plans/NNNN-title.md BEFORE coding. Mandatory sections: Context, Tasks, Critical files, Verification, Out of scope. Wait for approval. | "Use **plan-first-substantive-changes**." or fires automatically on scope | — |
| **post-sse-via-fetch-readablestream** | Claude Code + Cursor | [`skills/engineering/post-sse-via-fetch-readablestream/`](skills/engineering/post-sse-via-fetch-readablestream/) | EventSource is GET-only. For POST endpoints with SSE: fetch() + ReadableStream + manual frame parser. ~80 lines. Same {event,data} shape as streamSSE(). | "Use **post-sse-via-fetch-readablestream**." or "my SSE endpoint requires POST" | tool-calling-approval-reentry |
