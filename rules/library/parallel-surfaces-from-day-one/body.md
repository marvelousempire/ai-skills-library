...
- **"The backend is in the same monolith as the user web"** — for a personal local tool this is fine (see [`app-launch-workflow`](app-launch-workflow.mdc)). For anything that will have multiple clients, split it day one.
...
- Personal local tools — one user, one Mac, one purpose (see [`app-launch-workflow`](app-launch-workflow.mdc) for the simpler single-surface pattern).
...
- [`app-launch-workflow`](app-launch-workflow.mdc) — single-surface personal-tool pattern (the opposite case)
- [`go-live-path`](go-live-path.mdc) — every shipped feature ends with migrate / deploy / smoke-test
- [`dev-discipline`](dev-discipline.mdc) — session opener / closer rituals that keep multi-surface parallel work honest
